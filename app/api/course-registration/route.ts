...
export async function POST(request: NextRequest) {
  try {
    ...
    let destination = "";
    try {
      destination = courseWebhook ? await dedicated(courseWebhook, record) : await lead(leadWebhook!, record);
    } catch (primaryError) {
      if (courseWebhook && leadWebhook) destination = await lead(leadWebhook, record);
