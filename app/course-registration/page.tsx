...
function PageContent() {
  ...
  const batches = useMemo<Batch[]>(
    () => course.firstBatch ? weeklyBatches(course.firstBatch) : [{ id: "interest-list", label: "Interest Registration — Date to be confirmed" }],
    [course]
  );
  ...
  useEffect(() => {
    setForm((state) =>
      batches.some((item) => item.id === state.batchId)
        ? state
        : { ...state, batchId: batches.some((item) => item.id === requestedBatch) ? requestedBatch : batches[0].id }
    );
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(submissionError instanceof Error ? submissionError.message : "Unable to submit the registration.");
    } finally {
      setSubmitting(false);
    }
  }
          ...
          <section className="py-16">
            <div className="container">
              <div className="mx-auto max-w-3xl border bg-white p-8 text-center shadow-xl md:p-12">
                ...
                <a href={waUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-[#25D366] px-7 py-4 text-sm font-bold uppercase tracking-wider text-white">
                  <MessageCircle className="h-5 w-5" />Send Details on WhatsApp
                </a>
                <p className="mt-4 text-xs leading-6 text-black/55">A paid place is confirmed only after payment verification and written acknowledgement.</p>
              </div>
            </div>
          </section>
        </main>
      ...
      <main className="pt-28 md:pt-36">
        <section className="border-b pb-14 md:pb-20">
          <div className="container text-center">
            <p className="text-xs font-bold uppercase tracking-[0.25em] text-[oklch(0.60_0.08_65)]">Qimen Strategy Academy · 启明遁甲研修</p>
            <h1 className="mt-4 text-4xl font-bold md:text-6xl">Course Registration & PayNow</h1>
            <p className="mx-auto mt-5 max-w-3xl leading-8 text-black/65">Choose a course and batch. Qi Men foundation classes run on selected Saturdays and Sundays from 19 September 2026 onward.</p>
          </div>
        </section>
            ...
            <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.9fr_1.1fr]">
              <aside className="bg-black p-7 text-white md:p-10">
                ...
                <div className="mt-7 space-y-4 text-sm leading-7">
                  <p><strong>Batch:</strong><br />{batch.label}</p>
                  <p><strong>Location:</strong><br />{LOCATION}</p>
                  {course.fee && <p><strong>Fee:</strong><br /><span className="text-3xl text-[oklch(0.78_0.12_70)]">S${course.fee}</span> per participant</p>}
                </div>
                {course.fee && (
                  <div className="mt-7 border border-white/20 bg-white/5 p-5 text-center">
                    ...
                    <img src="/paynow-uen-qr.svg" alt="PayNow QR code for QIMING FENG SHUI WISDOM PTE. LTD." className="mx-auto mt-4 w-full max-w-[260px] bg-white p-2" />
                    <p className="mt-4 text-sm font-semibold">{COMPANY}</p>
                    <div className="mt-2 flex items-center justify-center gap-3">
                      <strong>UEN: {PAYNOW_UEN}</strong>
                      <button type="button" onClick={copyUen} className="inline-flex items-center gap-1 border border-white/25 px-2 py-1 text-xs">
                ...
                <div className="mt-7 border-t border-white/15 pt-5 text-xs leading-6 text-white/55">
                  ...
                  <Link href="/course-policy" className="mt-2 inline-block text-[#d6ad63] underline underline-offset-4">Read Course Registration Policy</Link>
                </div>
              </aside>
              ...
              <form onSubmit={submit} className="border bg-white p-6 shadow-lg md:p-8">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <label className="md:col-span-2">
                    ...
                    <select className={inputClass} value={form.courseId} onChange={(e) => changeCourse(e.target.value)}>
                      {COURSES.map((item) => <option key={item.id} value={item.id}>{item.name}｜{item.zh}</option>)}
                    </select>
                  </label>
                  <label className="md:col-span-2">
                    <span className={labelClass}>Course Batch *</span>
                    <select required className={inputClass} value={form.batchId} onChange={(e) => update("batchId", e.target.value)}>
                      {batches.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
                    </select>
                    {course.firstBatch && <small className="mt-2 block text-black/50">Future Saturday–Sunday batches are generated automatically.</small>}
                  </label>
                  <label>
                    <span className={labelClass}>Registrant Name *</span>
                    <input required className={inputClass} value={form.registrantName} onChange={(e) => update("registrantName", e.target.value)} />
                  </label>
                  <label>
                    <span className={labelClass}>WhatsApp / Phone *</span>
                    <input required className={inputClass} value={form.phone} onChange={(e) => update("phone", e.target.value)} placeholder="+65 ..." />
                  </label>
                  <label className="md:col-span-2">
                  ...
                  <label>
                    ...
                    <select className={inputClass} value={form.participantCount} onChange={(e) => update("participantCount", Number(e.target.value))}>
                      {[1, 2, 3, 4, 5, 6, 7, 8].map((number) => <option key={number}>{number}</option>)}
                    </select>
                  </label>
                  <div>
                    <span className={labelClass}>Estimated Total</span>
                    <div className={`${inputClass} bg-[oklch(0.98_0.01_75)] font-semibold`}>{total ? `S$${total.toLocaleString("en-SG")}` : "To be confirmed"}</div>
                  </div>
                  <label className="md:col-span-2">
                    <span className={labelClass}>Participant Full Name(s) *</span>
                    <textarea required rows={4} className={inputClass} value={form.participantNames} onChange={(e) => update("participantNames", e.target.value)} placeholder="One participant name per line" />
                  </label>
                  {course.fee && (
                    <>
                      <label>
                        <span className={labelClass}>Payment Status *</span>
                        <select className={inputClass} value={form.paymentStatus} onChange={(e) => update("paymentStatus", e.target.value)}>
                      ...
                      <label>
                        ...
                        <input className={inputClass} value={form.amountPaid} onChange={(e) => update("amountPaid", e.target.value)} />
                      </label>
                      <label className="md:col-span-2">
                        <span className={labelClass}>PayNow Reference</span>
                        <input className={inputClass} value={form.paymentReference} onChange={(e) => update("paymentReference", e.target.value)} />
                      </label>
                    </>
                  )}
                  <label className="md:col-span-2">
                    <span className={labelClass}>Notes</span>
                    <textarea rows={3} className={inputClass} value={form.notes} onChange={(e) => update("notes", e.target.value)} placeholder="Learning goals, language needs or special arrangements" />
                  </label>
                </div>

                <label className="mt-6 flex items-start gap-3 text-sm leading-6 text-black/65">
                  <input type="checkbox" required checked={form.acceptedPolicy} onChange={(e) => update("acceptedPolicy", e.target.checked)} className="mt-1 h-4 w-4 accent-[oklch(0.60_0.08_65)]" />
                  <span>
                ...
                <button disabled={submitting} className="mt-7 inline-flex w-full items-center justify-center gap-2 bg-[oklch(0.60_0.08_65)] px-7 py-4 text-sm font-bold uppercase tracking-wider text-white disabled:opacity-60">
                  {submitting ? <><Loader2 className="h-4 w-4 animate-spin" />Submitting</> : <><CheckCircle2 className="h-4 w-4" />Submit Course Registration</>}
                </button>
                <p className="mt-4 text-xs leading-6 text-black/50">Registration is recorded in our backend. A paid place is confirmed only after payment verification and written acknowledgement.</p>
              </form>
            </div>
          </div>
        </section>
